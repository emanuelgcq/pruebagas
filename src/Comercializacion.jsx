...
import { PreCierre, CarteraDemo, PagosDemo } from "./ComercializacionExtra.jsx";
...
export default function Comercializacion({
  ...
  const nav = [
    ...
    { id: "control", label: "Control del período", icon: CheckCircle2 },
    { id: "cartera", label: "Cartera", icon: Landmark },
    { id: "pagos", label: "Pagos y conciliación", icon: TrendingUp },
    { id: "ad", label: "Seguimiento distribución", icon: ClipboardList, badge: pendAD + abiertas },
    { id: "logistica", label: "Planificación logística", icon: Route, badge: rutasDistribucion.filter((r) => r.estadoRuta !== "ENTREGADA").length },
    ...
    <div className={`gl ${doc ? "printing" : ""}`}>
      ...
      <main className="main">
        ...
        <div className="body">
          ...
          {vista === "control" && <PreCierre {...{facV,solV,boletas:bopV,existencias,compromisos,disponibles,rutasDistribucion,cdtF,periodoCerrado,setPeriodoCerrado,onExport:expCierre}} />}
          {vista === "cartera" && <CarteraDemo solicitudes={solV} />}
          {vista === "pagos" && <PagosDemo solicitudes={solV} />}
          {vista === "ad" && <VistaAD {...{ solV, setDoc, boletas, facturas }} />}
          {vista === "logistica" && <VistaLogistica rutas={rutasDistribucion} />}
...
function VistaLogistica({ rutas = [] }) {
  const total = rutas.reduce((a,r)=>a+cilindrosFila(r),0);
  ...
  const kg = rutas.reduce((a,r)=>a+kgFila(r),0);
  return <>
    <div className="kpis">
      <Kpi label="Planificados 14/08" valor={`${num(total)} cilindros`} pie="misma planificación del módulo Distribución" tono="azul" />
      <Kpi label="Fuerza propia" valor={num(fuerza)} pie={`${total ? (fuerza/total*100).toFixed(1) : 0}% del plan`} tono="verde" />
      <Kpi label="EPSDC" valor={num(epsdc)} pie={`${total ? (epsdc/total*100).toFixed(1) : 0}% del plan`} tono="ambar" />
      <Kpi label="GLP comprometido en plan" valor={`${num(kg)} kg`} pie={`${num(kgALitros(kg))} L con factor 0,540 kg/L`} tono="gris" />
    </div>
    <div className="inv-regla"><Route size={17}/><div><b>Vista espejo de Distribución.</b><span>Estas cifras no se vuelven a cargar en Comercialización: se leen de la misma planificación compartida. Si Distribución cambia una unidad o el conductor registra una entrega, el estado cambia aquí también.</span></div></div>
    <section className="card">
      <div className="card-h"><div><h2>Planificación logística compartida</h2><span className="card-note">AD · comuna · comunidad · unidad · fuerza propia/EPSDC · estado</span></div></div>
      <div className="scroll"><table className="tbl"><thead><tr><th>AD</th><th>Comuna</th><th>Comunidad</th><th>10 kg</th><th>18 kg</th><th>27 kg</th><th>43 kg</th><th>Total</th><th>Unidad</th><th>Transportista</th><th>Estado</th></tr></thead><tbody>
        {rutas.map(r=><tr key={r.id}><td><b>{r.ad}</b></td><td>{r.comuna}</td><td>{r.comunidad}</td><td>{r.cilindros[10]}</td><td>{r.cilindros[18]}</td><td>{r.cilindros[27]}</td><td>{r.cilindros[43]}</td><td><b>{cilindrosFila(r)}</b></td><td>{r.unidad}</td><td>{unidadDistribucion(r.unidad).etiqueta}</td><td><span className={`chip ${r.estadoRuta === "ENTREGADA" ? "st-cul" : r.estadoRuta === "INCIDENCIA" ? "pri-alta" : r.estadoRuta === "EN_RUTA" ? "st-con" : "st-pag"}`}>{r.estadoRuta === "SIN_PLANIFICAR" ? "Sin planificar" : r.estadoRuta === "EN_RUTA" ? "En ruta" : r.estadoRuta === "ENTREGADA" ? "Entregada" : r.estadoRuta === "INCIDENCIA" ? "Incidencia" : "Asignada"}</span></td></tr>)}
      </tbody></table></div>
    </section>
  </>;
}
  ...
  return (
    <div className="cm-dash">
      ...
      <section className="cm-chart cm-money-chart">
        <ChartHead title="Gestión monetaria" subtitle="Principal · recaudación, facturación y cartera pendiente" right={<Legend items={[["Recaudado","#1C7A50"],["Facturado","#2D65B0"],["Recaudado sin despachar","#D08A24"],["Por recaudar","#7C8792"]]} />} />
      ...
      <div className="cm-chart-grid two">
        <section className="cm-chart">
          ...
          <div className="cm-inventory-total"><span>Inventario físico</span><b>{num(totalKg)} kg</b><span>Comprometido</span><b>{num(totalComp)} kg</b><span>Disponible</span><b>{num(totalDisp)} kg</b></div>
        </section>
      ...
      <div className="cm-chart-grid two">
        <section className="cm-chart">
          <ChartHead title="Movimiento de GLP" subtitle="Kg despachados físicamente vs kg comprometidos por pagos" right={<div className="cm-chart-badge">1 L = 0,540 kg</div>} />
          <GroupedBars data={glpSerie} series={[{key:"despachado",label:"Despachado",cls:"green"},{key:"comprometido",label:"Comprometido",cls:"amber"}]} />
          <div className="cm-money-foot two"><div><span>Despachado</span><b>{num(kgDespachadoPeriodo)} kg · {num(kgALitros(kgDespachadoPeriodo))} L</b></div><div><span>Comprometido</span><b>{num(kgComprometidoPeriodo)} kg · {num(kgALitros(kgComprometidoPeriodo))} L</b></div></div>
        </section>
      ...
      <div className="cm-mini-actions">
        ...
        <button onClick={()=>setVista("inventario")}><Gauge size={15}/><span>Disponible real</span><b>{num(totalDisp)} kg</b></button>
...
function InventoryBars({cdts,existencias,compromisos,disponibles}) { return <div className="cm-inv-list">{cdts.map(c=>{const fis=Number(existencias[c.id]||0),comp=Number(compromisos[c.id]||0),disp=Math.max(0,Number(disponibles[c.id]||0)),cap=Math.max(c.capacidad||1,fis);return <div className="cm-inv-row" key={c.id}><div className="cm-inv-name"><b>{c.corto}</b><span>{num(fis)} kg físicos</span></div><div className="cm-inv-track"><div className="cm-inv-disp" style={{width:`${Math.min(100,(disp/cap)*100)}%`}}/><div className="cm-inv-comp" style={{width:`${Math.min(100,(comp/cap)*100)}%`}}/></div><div className="cm-inv-val"><b>{num(disp)} kg</b><span>disponibles</span></div></div>})}</div>; }
...
function VistaFacturas({ facturas, setDoc, setModal, onExport }) {
  ...
  return (
    <>
      ...
      <section className="card">
        <div className="card-h"><div><h2>Control documental</h2><span className="card-note">Casos visuales de corrección fiscal sin eliminar el documento original.</span></div></div>
        <div className="scroll"><table className="tbl"><thead><tr><th>Documento original</th><th>Estado</th><th>Documento relacionado</th><th>Motivo</th><th>Fecha</th></tr></thead><tbody><tr><td className="mono strong">U-00000124</td><td><span className="chip pri-alta">ANULADA</span></td><td className="mono">NC-00000024</td><td>Error de cantidad · documento conservado en histórico</td><td>11/08/2026</td></tr><tr><td className="mono strong">U-00000126</td><td><span className="chip st-ad">SUSTITUIDA</span></td><td className="mono">U-00000131</td><td>Corrección de datos fiscales del receptor</td><td>12/08/2026</td></tr></tbody></table></div>
      </section>
      <section className="card">
        <div className="card-h">
  ...
  return (
    <>
      ...
      <section className="card">
        ...
        <div className="scroll"><table className="tbl"><thead><tr><th>Fecha / hora</th><th>Movimiento</th><th>Documento</th><th>Origen</th><th>Destino</th><th className="r">Entrada kg</th><th className="r">Salida kg</th><th className="r">Compromiso kg</th><th className="r">Saldo físico kg</th><th>Estado</th></tr></thead><tbody>
          <tr><td>14/08 · 06:12</td><td><span className="tag">Recepción GLP</span></td><td className="mono">REC-140826-01</td><td>Abastecimiento</td><td>Jacinto Lara</td><td className="r ok-num">+12.500</td><td className="r">—</td><td className="r">—</td><td className="r mono strong">{num((existencias["CDT-01"]||0)+12500)}</td><td><span className="pago-ok">Confirmada</span></td></tr>
          <tr><td>14/08 · 07:21</td><td><span className="tag warn">Compromiso por pago</span></td><td className="mono">PAG-8439182</td><td>Comercialización</td><td>Reserva lógica</td><td className="r">—</td><td className="r">—</td><td className="r warn-num">+180</td><td className="r mono strong">{num(existencias["CDT-01"]||0)}</td><td><span className="chip st-ad">Sin salida física</span></td></tr>
          <tr><td>14/08 · 08:04</td><td><span className="tag alt">Transferencia</span></td><td className="mono">TRF-140826-03</td><td>Jacinto Lara</td><td>CDT Palavecino</td><td className="r">—</td><td className="r neg">−1.500</td><td className="r">—</td><td className="r mono strong">{num(Math.max(0,(existencias["CDT-01"]||0)-1500))}</td><td><span className="pago-ok">Recibida</span></td></tr>
          <tr><td>14/08 · 10:32</td><td><span className="tag warn">Ajuste de conteo</span></td><td className="mono">AJ-140826-02</td><td>Conteo físico</td><td>Jacinto Lara</td><td className="r">—</td><td className="r neg">−12</td><td className="r">—</td><td className="r mono strong">{num(Math.max(0,(existencias["CDT-01"]||0)-1512))}</td><td><span className="chip st-pag">Justificado</span></td></tr>
        </tbody></table></div>
      </section>
      <section className="card">
        <div className="card-h">
  ...
  return (
    <>
      ...
      <section className="card">
        ...
        <div className="scroll"><table className="tbl"><thead><tr><th>EPSDC</th><th className="r">AD cerradas</th><th className="r">GLP kg</th><th className="r">Venta transportada Bs</th><th className="r">30% servicio Bs</th><th>Estado</th><th>Pago / soporte</th><th></th></tr></thead><tbody>{grupos.map((g,i)=>{const est=liqEstados[g.id]||"PENDIENTE";const pagada=est==="PAGADA";return <tr key={g.id}><td><div className="u-name">{epsdcOf(g.id).nombre}</div><div className="u-doc">{epsdcOf(g.id).rif}</div></td><td className="r mono strong">{g.ads}</td><td className="r mono">{num(g.kg)}</td><td className="r mono strong">{bs(g.venta)}</td><td className="r mono strong">{bs(g.venta*.30)}</td><td><span className={`chip ${pagada?"st-cul":est==="APROBADA"?"st-con":est==="REVISADA"?"st-ad":"st-pag"}`}>{est}</span></td><td>{pagada?<div><div className="u-name sm">Banco de Venezuela</div><div className="u-doc">Op. {`84${String(391820+i*731).padStart(7,"0")}`} · 16/08/2026</div></div>:<span className="muted">Pendiente de comprobante</span>}</td><td><button className="btn sm ghost" disabled={pagada} onClick={()=>avanzarLiq(g.id)}>{pagada?"Cerrada":"Avanzar estado"}</button></td></tr>})}</tbody></table></div>
      </section>
      <section className="card">
        <div className="card-h">
