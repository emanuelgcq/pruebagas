...
function SeguimientoUsuario({ mis }) {
  const latest = [...mis].sort((a,b)=>(b.fecha||0)-(a.fecha||0))[0];
  const current = latest || {};
  const delivered = current.estado === "CULMINADO";
  const withdrawn = Boolean(current.retiradoPorUsuario || current.retiroUsuario);
  const inAD = Boolean(current.ad) || current.estado === "EN_AD" || delivered;
  const stages = [
    ["Solicitud creada", true, current.id || "SOL-—", "12/08 · 09:22"],
    ["Pago confirmado", Boolean(current.pago), current.pago ? `${banco(current.pago.banco).nombre} · Op. ${current.pago.referencia}` : "Esperando pago", "12/08 · 11:13"],
    ["Esperando planificación", Boolean(current.pago), inAD ? "Planificación completada" : "Pendiente de AD", "13/08 · 08:15"],
    ["Incluido en AD", inAD, current.ad || "Sin AD", "14/08 · 07:24"],
    ["En camino a tu comuna", current.estado === "EN_AD" || delivered, current.unidad || "Unidad por asignar", "14/08 · 09:05"],
    ["Entregado a la comuna", delivered, delivered ? COM.nombre : "Pendiente de recepción comunal", "14/08 · 14:40"],
    ["Disponible para retirar", delivered, delivered ? "Tu bombona está disponible en la comuna" : "Se habilita al recibir la jornada", "14/08 · 15:10"],
    ["Retirado por el usuario", withdrawn, withdrawn ? "Retiro registrado por la comuna" : delivered ? "Pendiente de retiro por el ciudadano" : "Pendiente", "—"],
  ];
  const history = [
    ["14/08/2026", current.concepto ? cpt(current.concepto).corto : "Bombona 10 kg", COM.nombre, withdrawn ? "RETIRADA" : delivered ? "DISPONIBLE PARA RETIRO" : "EN PROCESO"],
    ["12/07/2026", "Bombona 10 kg", COM.nombre, "RETIRADA"],
    ["18/06/2026", "Bombona 18 kg", COM.nombre, "RETIRADA"],
    ["21/05/2026", "Bombona 10 kg", COM.nombre, "RETIRADA"],
  ];
  return <div className="pu-track"><style>{`
    .pu-track{display:flex;flex-direction:column;gap:16px}.pu-track-card{background:var(--paper);border:1px solid var(--line);border-radius:18px;padding:18px}.pu-track-head h2{margin:0;font-size:18px}.pu-track-head p{margin:5px 0 0;color:var(--ink-3);font-size:12px}.pu-stage-list{margin-top:18px}.pu-stage{display:grid;grid-template-columns:32px 1fr auto;gap:10px;min-height:62px;position:relative}.pu-stage:before{content:"";position:absolute;left:15px;top:28px;bottom:-5px;width:1px;background:var(--line)}.pu-stage:last-child:before{display:none}.pu-stage>i{width:30px;height:30px;border-radius:50%;display:grid;place-items:center;background:var(--line-2);color:var(--ink-3);font-style:normal;font-weight:800;font-size:11px}.pu-stage.done>i{background:#e8f5ed;color:#17623f}.pu-stage b,.pu-stage span{display:block}.pu-stage b{font-size:13px}.pu-stage span{font-size:11px;color:var(--ink-3);margin-top:3px}.pu-stage time{font-size:10px;color:var(--ink-3);white-space:nowrap}.pu-delivery-note{display:flex;gap:9px;background:#edf6f1;border:1px solid #d7e8de;border-radius:12px;padding:12px;color:#315e48;font-size:11px;line-height:1.45}.pu-hist{width:100%;border-collapse:collapse;font-size:12px}.pu-hist th,.pu-hist td{padding:10px;border-bottom:1px solid var(--line);text-align:left}.pu-hist th{font-size:10px;text-transform:uppercase;color:var(--ink-3)}.pu-state{display:inline-flex;padding:4px 7px;border-radius:99px;background:#e8f5ed;color:#17623f;font-size:9px;font-weight:800}.pu-state.wait{background:#fff2dd;color:#9d651a}@media(max-width:700px){.pu-stage{grid-template-columns:30px 1fr}.pu-stage time{grid-column:2}}
  `}</style>
    <section className="pu-track-card"><div className="pu-track-head"><h2>Seguimiento completo de tu ciclo</h2><p>GasLara diferencia la entrega a la comuna del retiro final de tu bombona.</p></div><div className="pu-stage-list">{stages.map((x,i)=><div className={`pu-stage ${x[1]?"done":""}`} key={x[0]}><i>{x[1]?<Check size={14}/>:i+1}</i><div><b>{x[0]}</b><span>{x[2]}</span></div><time>{x[3]}</time></div>)}</div></section>
    <div className="pu-delivery-note"><Users size={18}/><span><b>Importante:</b> “Entregado a la comuna” significa que GasLara culminó su entrega logística. Tu ciclo personal termina cuando la comuna registra que tú retiraste la bombona.</span></div>
    <section className="pu-track-card"><div className="pu-track-head"><h2>Historial de retiros</h2><p>Jornadas anteriores asociadas a tu contrato.</p></div><div className="scroll"><table className="pu-hist"><thead><tr><th>Fecha</th><th>Producto</th><th>Punto de retiro</th><th>Estado final</th></tr></thead><tbody>{history.map((h,i)=><tr key={i}><td>{h[0]}</td><td><b>{h[1]}</b></td><td>{h[2]}</td><td><span className={`pu-state ${h[3]!=="RETIRADA"?"wait":""}`}>{h[3]}</span></td></tr>)}</tbody></table></div></section>
  </div>;
}
